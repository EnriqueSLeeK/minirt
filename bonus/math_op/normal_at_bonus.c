...
/*   Updated: 2022/12/04 13:20:06 by ensebast         ###   ########.br       */
/*                                                                            */
/* ************************************************************************** */
...
void	normal_cn(t_elem *elem, t_tuple obj_p, t_tuple *tup)
{
	...
	dist = (obj_p.tup[0] * obj_p.tup[0])
		+ (obj_p.tup[2] * obj_p.tup[2]);
	if (dist < (elem->max * elem->max)
		&& obj_p.tup[1] >= (elem -> max - EPSILON))
		*tup = vector(0, 1, 0);
	else if (dist < (elem->min * elem->min)
		&& obj_p.tup[1] <= (elem -> min + EPSILON))
		*tup = vector(0, -1, 0);
	else
	{
		y = sqrt(dist);
		if (obj_p.tup[1] > 0)
			y *= -1;
		*tup = vector(obj_p.tup[0], y, obj_p.tup[2]);
	}
}
