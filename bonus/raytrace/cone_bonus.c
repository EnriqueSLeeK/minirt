...
/*   Updated: 2022/12/04 12:24:05 by ensebast         ###   ########.br       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_cap(t_ray *ray, double t, double cap)
{
	double	x;
	double	z;
...
static void	intersect_caps(t_elem *elem, t_ray *ray, t_intersect_list *i_list)
{
	...
	if (check_cap(ray, t[0], elem->min))
		add_intersect(i_list, elem, t[0]);
	if (check_cap(ray, t[1], elem->max))
...
void	intersect_cn(t_elem *elem, t_ray *ray, t_intersect_list *i_list)
{
	double		t[2];
	double		y[2];
	double		factor;
	...
	{
		if (fabs(bhask.a) < EPSILON && !cmp_float(0, bhask.b))
